...
frappe.query_reports["Building Fund Report"] = {
	"filters": [
		{
			"fieldname": "year",
			"label": __("Year"),
	...
	"onload": function(report) {
		...
		report.page.add_inner_button(__("Contributor Summary"), function() {
			...
			frappe.call({
				method: "stewardpro.stewardpro.report.building_fund_report.building_fund_report.get_contributor_summary",
				args: {
					filters: filters
				},
				callback: function(r) {
		...
		report.page.add_inner_button(__("Monthly Summary"), function() {
			let filters = report.get_values();
			frappe.call({
				method: "stewardpro.stewardpro.report.building_fund_report.building_fund_report.get_monthly_summary",
				args: {
					filters: filters
				},
				callback: function(r) {
